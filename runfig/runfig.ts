...
async function runfig(args: Args) {
  const markdown: Formatter = async ({ module, obj }) => {
    const processor = unified().use(remarkParse).use(remarkStringify);
    const { value } = await processor.process(obj);
    return `<!-- Auto-generated by runfig from ${module} -->\n\n${value}`;
  };
  const toml: Formatter = async ({ module, obj }) =>
    `# Auto-generated by runfig from ${module}\n\n${TOML.stringify(obj)}\n`;
  const yaml: Formatter = async ({ module, obj }) =>
    `# Auto-generated by runfig from ${module}\n\n${YAML.stringify(obj)}`;
  const json: Formatter = async ({ obj }) => JSON.stringify(obj, undefined, 2);
...
async function unfig(args: Args) {
  const markdown: Parser = async (data) => {
    const processor = unified().use(remarkParse).use(remarkStringify);
    const { value } = await processor.process(data);
    return value;
  };
  const toml: Parser = async (data) => TOML.parse(data);
  const yaml: Parser = async (data) => YAML.parse(data);
  const json: Parser = async (data) => JSON.parse(data);
  ...
  const ts: Formatter = async ({ module, obj }) =>
  ...
  for (const module of modules) {
    ...
    const obj = await parser(input);
    const output = await ts({ module, obj });
